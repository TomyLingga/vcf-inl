...
export default function TransportersPage() {
  ...
  const openCreate = () => {
    setEditing(null);
    setForm({ nama_transporter: "", kode: "", is_active: true });
    setError("");
    setShowModal(true);
  };

  const openEdit = (item: Transporter) => {
    setEditing(item);
    setForm({ nama_transporter: item.nama_transporter, kode: item.kode, is_active: item.is_active });
    setError("");
    setShowModal(true);
  };
  ...
  return (
    <div>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <div className="relative group">
            ...
            <div className="absolute right-0 mt-1 w-52 border border-border rounded-xl shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all z-50" style={{ background: "var(--bg-secondary)" }}>
              <button
                onClick={() => downloadImportTemplate("Transporter", ["nama_transporter *", "kode *", "is_active (Ya/Tidak)"], [
                  ["PT. Maju Jaya Logistik", "MJL", "Ya"],
                  ["CV. Cepat Sampai", "CPS", "Ya"],
                  ["PT. Artha Kencana Trans", "AKT", "Tidak"],
                ])}
                className="w-full text-left px-4 py-2 text-sm hover:bg-bg-card-hover first:rounded-t-xl flex items-center gap-2"
          ...
          <div className="modal-content" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h2 className="font-semibold text-base" style={{ color: "var(--text-primary)" }}>
                {editing ? "Edit Transporter" : "Tambah Transporter"}
              </h2>
            ...
            <form onSubmit={handleSave}>
              <div className="mb-4">
                ...
                <input
                  id="input-nama-transporter"
                  type="text"
                  className="form-input"
                  required
                  value={form.nama_transporter}
                  onChange={(e) => setForm((p) => ({ ...p, nama_transporter: e.target.value }))}
                />
              </div>
              <div className="mb-4">
                <label className="form-label">Kode *</label>
                <input
                  id="input-kode-transporter"
                  type="text"
                  className="form-input"
                  required
                  value={form.kode}
              ...
              <div className="mb-6 flex items-center gap-3">
                <input
                  id="check-active-transporter"
                  type="checkbox"
                  checked={form.is_active}
                  onChange={(e) => setForm((p) => ({ ...p, is_active: e.target.checked }))}
                  style={{ width: 16, height: 16 }}
                />
                <label htmlFor="check-active-transporter" className="text-sm" style={{ color: "var(--text-secondary)" }}>
                  Aktif
                </label>
              </div>
              <div className="flex gap-3 justify-end">
                ...
                <button id="btn-save-transporter" type="submit" className="btn btn-primary" disabled={saving}>
                  {saving ? (
                    <>
      ...
      <DeleteConfirmModal
        ...
        loading={deleting}
        message="Menghapus data transporter akan berpengaruh pada riwayat transaksi kendaraan yang terkait secara permanen. Lanjutkan?"
      />

      {isPrinting && (
        <PrintMasterTable
          title="Master Data — Transporter"
          subtitle="Daftar perusahaan transporter rekanan PT. Industri Nabati Lestari"
          headers={["Kode", "Nama Transporter", "Status"]}
          data={data.map(t => [t.kode, t.nama_transporter, t.is_active ? "Aktif" : "Nonaktif"])}
